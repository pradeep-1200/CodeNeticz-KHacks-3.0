...
function solveNumerical(question) {
    ...
    if (type === "percentage") {
        // Pattern: "What is X percent of Y?"
        ...
        // Pattern: "If 1 pen costs 10, how much do 4 pens cost?"
        // Detect: (qty1) (item) ... (cost1) ... (qty2)
        // Simple case: "1 pen costs 10. 4 pens?"
        const nums = normalized.match(/\d+(\.\d+)?/g);
        if (nums && nums.length >= 2) {
            // Heuristic strategies (simple rule based)
            // Assuming structure: "1 unit = X currency. Find Y units."
            // If 2 numbers, maybe "1 pen is 10. cost of 5?" -> we have 1, 10, 5.

            // Let's look for the price/cost format
            // "one pen costs 10 rupees" -> 1, 10
            // "how much do 4 pens cost" -> 4

            // We need to parse more intelligently.
            // Simplified logic for prototype: find single unit value, multiply by target quantity.

            // Regex for "cost of <n> is <price>"
            // Regex for "<n> <item> costs <price>"

            // Let's try to extract triples: (q1, p1) -> (q2, ?)
            // "if 1 pen costs 10"
            ...
            // Target quantity: "how much do 4 cost" or "cost of 4"
            const targetMatch = normalized.match(/(\d+)\s+([a-z]+)/);
            // This is risky, might match the first part again.
            // We need to match the number associated with the query.
            // "how much do 4 pens cost"

            // Let's use a simpler extraction for the specific user examples
            // "If one pen costs 10 rupees, how much do 4 pens cost?"
            // We have 1, 10, 4.
            // If we see 3 numbers and "1" is one of them, it's likely multiplication.
            const allNums = nums.map(n => parseFloat(n));

            if (allNums.length === 3 && allNums.includes(1)) {
                const quantity = allNums.find(n => n !== 1 && n !== unitPrice);
                // Wait, we don't know unitPrice yet if we just grab numbers.

                // Robust-ish parsing:
                // 1. Find the "1 unit = value" relationship.
                // 2. Find the "target units" query.

                // Let's rely on standard patterns for now.
                // "1 [item] costs [price]"
        ...
        // "A person walks 4 km in 1 hour. How far in 3 hours?"
        // Pattern: X units in 1 unit. Result in Y units?
        ...
        if (matches) {
            ...
            // Simple calculation assuming same day / chronological
            let mins1 = h1 * 60 + m1;
            let mins2 = h2 * 60 + m2;

            if (mins2 < mins1) mins2 += 12 * 60; // Wrap around 12 hour clock heuristic
